...
- 状态栏支持“重复上次区域”：恢复原显示器上的可移动、可缩放选区，显示器分辨率变化后仍按相对位置恢复。
- 状态栏支持 3、5、10 秒延时截图：先完成框选和标注，再确认复制、保存或钉图；倒计时期间可回到原应用准备画面，再按全局截图快捷键可取消。
...
- 评论箭头头部默认吸附目标边缘，拖离约 14 px 后可在框内外自由定位；自由头部不会因编辑文字、移动或缩放目标框而复位，双击头部可恢复自动指向。
- 工具栏固定按默认数字快捷键 `1–9`、字母快捷键 `E/P/H/T`、编辑与导出操作分组排列；自定义工具快捷键会保存在本机，重启和升级后继续生效，但不会改变图标顺序。
...
- OCR 同时识别文字、二维码和常见条码；文字可选择、翻译或复制，多个码可逐项选择复制，“复制全部”会保留文字和所有码内容。
- OCR 面板可随时切换“保留换行”；关闭后正文会合并成连续文本，设置在本机持续生效，二维码和条码内容不受影响。
- OCR 面板会跟随截图选区所在显示器出现，识别中或失败时不会复制占位文字。
...
- 长截图会根据首个有效滚动手势自动识别纵向或横向；支持双向追加、固定栏清理和动态页面渐进式重试。实时预览可在纵向拖动上下边缘、横向拖动左右边缘裁剪，并显示最终尺寸和一键重置。
- 横向长截图采用手动滚动，避免误触自动滚动；方向锁定后会忽略触控板的垂直抖动，向回滚动时可在已有长图的左侧继续补齐内容。
- 设置中的长截图诊断会分别汇总纵向和横向的完成数、采纳率与失败帧；诊断不包含截图内容、识别文字、应用名或文件路径。
- `Cmd + C` 复制最终截图到剪切板。
...
- 首次启动提供权限设置指南：先解释必需的屏幕录制权限，辅助功能保持可选；授权后可重新检查或一键重启，并直接开始第一次截图。已有授权的老用户升级时不会被打断。
