...
- `Cmd + C` 复制最终截图到剪切板。
- `Space` 保存图片到设置目录；默认仍为 `~/Downloads`。
- 本地保存支持 PNG、JPEG、HEIC、TIFF；JPEG/HEIC 可调质量，支持自定义保存目录和 `{type}`、`{date}`、`{time}`、`{random}` 文件名模板。
- 快速访问卡片和历史记录缩略图支持直接拖到 Finder、邮件、聊天或设计软件；已保存文件保留原格式和文件名，内部截图自动生成可读文件名。
- 快速访问卡片和历史记录提供原生 macOS 分享面板，可直接发送到隔空投送、邮件、信息、备忘录及系统中已注册的其他目标。
- “完成后”设置可控制保存图片后是否同时复制，以及截图或录屏后是否显示快速访问；关闭快速访问后仍会在操作屏幕显示轻量反馈。
- 录制中可从顶部控制条或状态栏暂停、继续和停止；再次按当前截图快捷键仍会停止。暂停片段会无缝合并，视频保存到设置目录。
...
- 状态栏菜单提供屏幕录制、辅助功能、麦克风权限状态，提供打开系统设置、导出设置和退出入口；权限修改后请退出并重新打开 App。
